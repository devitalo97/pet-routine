...
import { PetFindOneUseCase } from "#/core/application/use-case/pet.find-one.use-case";
...
class AppBootstrap {
	build(config: Config) {
		...
		const petFindOneUseCase = new PetFindOneUseCase(petRepository);
		return { petCreationUseCase, petFindOneUseCase };
	}
}
